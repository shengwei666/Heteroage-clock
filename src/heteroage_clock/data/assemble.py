...
Updates:
- [Critical Fix] Implemented CHUNKED IMPUTATION to prevent OOM hang.
...
def filter_and_impute(df: pd.DataFrame, features_to_keep: list = None, impute_strategy: str = 'median') -> pd.DataFrame:
    """
    Filters features and imputes missing values using CHUNKED processing.
    THIS IS THE CRITICAL FIX FOR OOM.
    """
    # 1. Feature Filtering
    if features_to_keep:
    ...
    # Exclude metadata from imputation
    cols_to_impute = [c for c in numeric_cols if c not in ['age', 'sample_id', 'Sex_encoded', 'Age', 'project_id']]

    if not cols_to_impute:
        return df_filtered

    # 3. All-NaN Guard (Log only)
    log(f"  > Starting Chunked Imputation for {len(cols_to_impute)} columns...")

    # 4. Chunked Imputation (The Fix)
    # Process 5000 columns at a time to keep RAM usage negligible
    chunk_size = 5000
